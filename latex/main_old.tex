...
For example, in Turkish-Penn the feature Number appears frequently with the value Singular (86\% of its occurrences), whereas Tense is more evenly distributed (Past 50\%, Present 45\%, Future 5\%).
From an information-theoretic perspective, Tense as a category is more informative than Number, as its values are less predictable. 
For a given token, if the value of Tense is Past, that is less surprising than if it was Future.
