...
The SPLOM visualisations compare different measurements of the same data in a  pairwise fashion.\footnote{There are four datasets included in our study that do not contain regular tokens. They are: Arabic-NYUAD, English-GUMReddit, Japanese-BCCWJ and
Japanese-BCCWJLUW. The tokens are annotated for dependencies and features, but the tokens themselves are missing or very irregular. The lemma annotation is also irregular. For this reason, we do not include a few of the metrics for these four dataset (TTR, LTR, Number of types and mean surprisal of token).}.
...
A comparison of their metric with our two representative metrics can be seen in Figure \ref{fig:SPLOM_metrics_external}.
...
The correlations between our metrics and the Grambank metrics are typically weak ($0.45^*-0.48^*$) or not significant (Figures \ref{fig:SPLOM_metrics_external} and \ref{fig:SPLOM_metrics_external_PUD}).
...
\begin{figure}
    ...
    \caption{SPLOM representative custom metrics, type-token ratio (TTR), Grambank metrics and mean feature entropy (all UD datasets).}
...
\begin{figure}
    ...
    \caption{SPLOM representative custom metrics, type-token ratio (TTR), Grambank metrics and mean feature entropy (PUD datasets only).}
...
With one exception, they all strongly correlate with each other ($>=0.7^*$, see Figures  \ref{fig:SPLOM_custom_metrics} and \ref{fig:SPLOM_custom_metrics_PUD}). 
...
\begin{enumerate}
\item Collapse the files of each UD dataset (e.g. dev, test and train) to one table with one token per row
...
\item Resolve multi-word tokens. Some datasets will split tokens which are taken as representing multiple words into separate tokens. For example contractions such as: ``don't'' in English could be split into ``do'' and ``n't''. Within the UD-project separate words that belong to a multi-word token (e.g. ``do'' and ``n't'') can be included as separate tokens and marked for syntactic dependencies, part-of-speech (UPOS), lemma and morphological features. If that schema is applied, then the multiword token (e.g. ``don't) is unmarked for the same annotations. In this project, we want to analyse the surface forms - therefore we are re-assigning the annotations to the multi-word (e.g. ``don't'') and removing the component words (e.g. ``do'' and ``n't''). This may also be beneficial for comparability as multi-words may represent highly grammaticalised items that may be treated differently across datasets (sometimes split, sometimes not). By not analysing them further, we may be avoiding issues of uneven level of granularity of analysis.\footnote{For more details on multi-word tokens in UD, see \href{https://universaldependencies.org/format.html\#words-tokens-and-empty-nodes}{https://universaldependencies.org/format.html\#words-tokens-and-empty-nodes}.}
...
\item Remove datasets with less than 13,000 tokens.
\item Define lemmas as a combination of lemma + UPOS, so that lemmas that are spelled the same but belong to different UPOS can be distinguished. \textit{Example:} the noun ``mark'' and the verb ``mark'' in English are defined as having the lemmas ``mark\_NOUN'' and ``mark\_VERB'' as opposed to both potentially being assigned to the same lemma ``mark''.
\item Determine the full set of morphological feature categories per lemma. \textit{Example:} the token `sebuah' in Table \ref{tab:unassigned_ex} has features Definite and PronType; supposing there were another token of the same lemma in the dataset that had a value for the category Number, then the full set of categories for `sebuah' would be Definite, Number and PronType.
\item Remove features that are not related to morphology: ``Abbr'' (abbreviations), ``Typo'' (whether or not a token has a typo) and ``Foreign'' (whether or not a token is deemed as in a foreign language)
\item Assign a dummy value to unassigned morphological categories for each token, given the other available feature categories for tokens of the same lemma. \textit{Example:} for `sebuah' we insert the dummy feature Number=unassigned.
...
\section{Comparison of metrics}\label{sec:appendixComparison}

Several reviewers asked for a detailed comparison between our metric and that of \citet{ccoltekin2023complexity} (C\&R).
Here we list all the differences of which we are aware.
In each case we believe our methodology is an improvement and we indicate exactly how.

\subsection{C\&R remove all empty lemmas and tokens; we don't}

C\&R remove tokens whose lemma is empty or whose token is empty.
However, some tokens are empty because their lemma is understood to be identical to the token.
We treat these cases as genuine tokens. 
...
For example, in UD\_Chinese-GSDSimp there are tokens such as `50\%' whose morphological feature list has a value `NumType=Card', contrasting with other NUM tokens with feature values `NumType=Ord'. 
We include these in order to avoid artificially restricting the kinds of tokens whose morphological features are involved in the calculation of the metric. 
...
We have avoided that question here, instead removing only those parts of speech that clearly play no role in morphological complexity e.g. punctuation.

\subsection{C\&R sample; we use the entire dataset}

C\&R sample 20,000 tokens (with replacement) 100 times per dataset, yielding an average value for morphological feature entropy.
Their justification is that this aids comparability: two datasets, one with 50,000 tokens and the other with 1.5 million, would yield different orders of magnitude of datapoints for analysis. 
To ensure all datasets provide the same amount of data, a common sampling of this kind is required.

Whether or not this justification is appropriate for their other metrics, it does not hold for morphological feature entropy.
