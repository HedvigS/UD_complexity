...
Research to date has primarily focussed on relating language complexity to contact languages \citep{mcwhorter_2003} and population size and/or proportion of non-native speakers (inter alia \citet{wraygrace2007, dahl2004growth, lupyandale2010, bentz2013languages, bentz2015adaptive, raviv2019larger, koplenig2019language, shcherbakova2023societies}).
...
Some of these measurements correlate and may track the same/similar phenomena \citep{bentz2016comparison, bentz_complexity_2023, ccoltekin2023complexity}, but it would be wrong to conclude that every measurement of `linguistic complexity' tracks the same thing \citep{lupyan2024cautionary}.
...
\begin{table}[ht]
    ...
    \begin{tabular}{p{3cm}p{5cm}p{3cm}p{2cm}p{2cm}}  % Adjust column alignment as needed
        ...
        \midrule
       boundedness/fusion   & how much grammatical material is morphologically bound &  \cite{skirgard_grambank_2023-1}; \cite{shcherbakova2023societies}; \citet{lupyandale2010} \\ 
         \midrule
        type-token ratio (TTR) & how many unique words in relation to all words (more morphology = more unique words)  & \cite{kettunen2014can}; \citet{ccoltekin2023complexity}\\ 
               \midrule
 synthesis & morpheme to word ratio  & \citet{easterday_syllable_2021} \\
                     \midrule
     information      & how many grammatical distinctions a grammar makes possible & \cite{shcherbakova2023societies};  \cite{skirgard_grambank_2023-1}; \citet{lupyandale2010}; \citet{ccoltekin2023complexity}\\
\midrule
informativity/prediction accuracy/contextual predictability  & how accurately the next item can be predicted & \cite{frank2011insensitivity}; \citet{cohen-priva_phone_2008}; \citet{levshina_frequency_2022} \\    \midrule
...
regularity & the consistency of rules, for example in paradigms &  \cite{round_cognition_2022}‚ \cite{ackerman_parts_2009}; \cite{cotterell2019complexity} \\    \midrule
compositionality  & the extent to which the meaning of larger units is built from the meanings of smaller parts &  \cite{wraygrace2007} \\    \midrule
Kolmogorov complexity  & how much a computer algorithm can compress language material &  \cite{juola1998measuring,ehret_information-theoretic_2016,moscoso_del_prado_martin_putting_2004,sagot_comparing_2013} \\    \midrule
...
\bottomrule
    \end{tabular}
    \caption{Non-exhaustive table of different language complexity metrics found in the research literature.}g
...
In this sense, languages with more unique word forms and richer paradigmatic patterns to learn (and thereby more potential forms to process online) instantiate one dimension of language complexity (c.f. \citet{baerman_understanding_2015}, \citet{bentz2016comparison}, \citet{dahl2004growth}).
...
However, a language can have a rich morphological paradigm in its ruleset, but when one studies usage it turns out several of the forms are rarely uttered.
...
For example, in Spanish there are three distinctions in the demonstrative system (\emph{este}/\emph{ese}/\emph{acquel} \citet[87]{butt_new_2019}\footnote{This is the masculine singular series. The markers differ further for feminine, neuter and plural.}). 
However, out of these the first two are used the most. 
In the Universal Dependencies (UD) dataset Spanish-AnCora \citep{taule_ancora_2008}, \emph{acquel} is used 5\% of the time, \emph{ese} 26\% and \emph{este} 69\%,.
...
Because the frequencies of available morphological forms may have an impact on the difficulty of learning and processing the language, we can approach linguistic complexity through a corpus study of morphology.
...
%%% SFM end word chopping
%Several studies have sought to link sociocultural and demographic features with the various facets of linguistic complexity \citep{wraygrace2007,lupyandale2010,shcherbakova2023societies,dale_lupyan_2012}.
...
A similar measure was previously defined by \citet{sproat_database_nodate}.
...
\citet{sproat_database_nodate} obtain these probabilities by estimating them; however, they do not fully describe how the estimate is produced.
...
Whereas \citet{ccoltekin2023complexity} calculate frequencies on a sample-by-sample basis, and \citet{sproat_database_nodate} estimate frequencies in an unknown manner, we calculate frequencies across the entire corpus for each UD dataset.
...
\begin{figure}
    ...
    \caption{SPLOM of custom metrics (all UD datasets). Since all eight measures are variations on the theme `mean surprisal per token', the correlations are all strong or very strong. The third and sixth measures here are used for comparison with other metrics in Figures \ref{fig:SPLOM_other_metrics} and \ref{fig:SPLOM_metrics_external_CR}, \ref{fig:SPLOM_metrics_external_grambank}}
...
\begin{figure}
    ...
    \caption{SPLOM of custom metrics (PUD datasets only). Since the PUD datasets are more uniform in genre, the correlations are stronger than their counterparts in Figure \ref{fig:SPLOM_metric_custom}. The third and sixth measures here are used for comparison with other metrics in Figures \ref{fig:SPLOM_other_metrics_PUD} and \ref{fig:SPLOM_metrics_external_CR}, \ref{fig:SPLOM_metrics_external_grambank}}
...
These distributions are similar to the morphological patterns of isolating//agglutinating/fusional languages often referenced in linguistic typology (cf. \cite{comrie_language_1989} and \cite{croft1990}), with `hot spots' in eastern Europe and Central Asia and lower values in Southeast Asia. 
...
Third, both of our comparison metrics correlate more strongly with Grambank's Fusion metric **** BUT GRAMBANK METRICS ARE NO LONGER IN THIS PLOT! ****, suggesting our measures better capture this aspect of a language's grammar (though see the next section).
...
See section Analysis: PCA in \citet{skirgard_grambank_2023-1} for more details on the metrics.
...
Furthermore, our metrics correlate more strongly with Grambank's Fusion measure (Figure \ref{fig:SPLOM_metrics_external_grambank}).
