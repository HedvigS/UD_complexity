...
%  \subtitle{...}
  ...
  The emergence of annotated corpora collections in many different languages opens up new avenues for research in linguistic typology, enabling researchers to take into account more nuances of language use than previously.
    ...
    We find that languages vary in how much information is encoded in morphology and that the variation correlates moderately with other metrics previously used to quantify ``linguistic complexity''.
...
Language complexity is a multifaceted concept that can be approached in different ways.
A widely used distinction is between \emph{absolute} and \emph{relative} complexity \citep{miestamo_complexity_2006, miestamo_grammatical_2008}.
Absolute complexity concerns properties of the language system itself, such as the size of paradigms, whereas relative complexity concerns difficulty for particular learners or speakers.
...
Different theoretical approaches target different dimensions of linguistic complexity, yielding measurements that may correlate \citep{bentz2016comparison, bentz_complexity_2023, ccoltekin2023complexity}  but can also diverge \citep{lupyan2024cautionary}.
For example, complexity can be operationalized in terms of paradigmatic predictability \citep{round_cognition_2022, ackerman_parts_2009, cotterell2019complexity} or syntactic dependency length \citep{gibson1998linguistic, liu2008dependency}, which need not align.
...
Within morphological complexity, two commonly studied dimensions are enumerative complexity (E-complexity), capturing the number of overt morphological distinctions \citep{cotterell2019complexity} and integrative complexity (I-complexity), capturing implicative relations within paradigms \citep{kusters_linguistic_2003, ackerman_morphological_2013, cotterell2019complexity}.
Our study focuses on the surprisal of morphological features in corpora and can be seen as a usage-based application of E-complexity, sensitive to distributional asymmetries in use.
...
The table brings together work based on theoretical analyses, cross-linguistic questionnaire-based surveys (e.g. WALS \citep{dryer_wals_2013}; Grambank \citep{skirgard_grambank_2023-1}), paradigms (e.g. Paralex datasets \citep{beniamine_paralex_2023, elsner_computational_2024}, Parabank \citep{quinn_evolutionary_2025}) and corpora (e.g. Universal Dependencies \citep{UD_2.14}), experiments, and simulation studies.
...
\begin{table}[ht]
 ...
 synthesis & morpheme to word ratio  & \citet{sapir_introduction_1921}; \citet{greenberg_quantitative_1960}; \citet{bybee_morphology_1985}; \citet{easterday_syllable_2021} \\
                     \midrule
     E-complexity  & how many grammatical distinctions there are  & \citet{greenberg_quantitative_1960}; \cite{shcherbakova2023societies};  \cite{skirgard_grambank_2023-1}; \citet{lupyandale2010}; \citet{ccoltekin2023complexity}\\
\midrule
informativity/ prediction accuracy/ contextual predictability  & how accurately the next item in a string of langauge production can be predicted & \cite{frank2011insensitivity}; \citet{cohen-priva_phone_2008}; \citet{levshina_frequency_2022}; \citet{cotterell_are_2020} \\    \midrule
redundancy  & how much information is repeated & \cite{leufkens2023measuring}\\    \midrule
systematicity/ transparency  & the degree of consistency of form to meaning patterns & \cite{raviv2019larger}; \cite{hengeveld2018transparent}; \citet{miestamo_complexity_2006} \\    \midrule
I-complexity / paradigm complexity & predictability of forms within paradigms &  \citet{bybee_morphology_1985}; \cite{round_cognition_2022}; \cite{ackerman_parts_2009}; \citet{ackerman_morphological_2013}; \cite{cotterell2019complexity}; \citet{sagot_comparing_2013}; \citet{moscoso_del_prado_martin_putting_2004} \citet{beniamine_one_2021}; \citet{bonami_joint_2016}\\    \midrule
...
paradigm syncretism &  degree to which distinct morphosyntactic feature combinations share surface forms & \citet{quinn_evolutionary_2025};  \\    \midrule
Kolmogorov complexity / description length & Compactness of descriptions; amount of information required to encode the data, typically approximated via compression or coding-based measures) &  \cite{kolmogorov_three_1965, juola1998measuring, dahl2004growth, miestamo_feasibility_2006, miestamo_grammatical_2008, aceves2024human,  sagot_comparing_2013, ehret_information-theoretic_2016}  \\    \midrule
...
As with many other studies, we take morphological complexity to be related to the concomitant difficulty of language processing and learning \citep{findlay_effects_1995, cohen-mimran_effect_2013, mehravari_effects_2015, bentz_learning_2016}.
...
Many studies have contributed to this discussion by studying the grammatical rules of different languages (cf. \citet{lupyandale2010}, \citet{shcherbakova2023societies}).
However, a language can have a rich morphological paradigm in its rule-set, but when one studies usage it turns out several of the forms are rarely uttered.
...
For example, in Spanish there are three distinctions in the demonstrative system (\emph{este}/\emph{ese}/\emph{aquel} \citet[87]{butt_new_2019}\footnote{This is the masculine singular series. The markers differ further for feminine, neuter and plural.}). 
However, out of these the first two are used the most. 
In the Universal Dependencies (UD) dataset Spanish-AnCora \citep{taule_ancora_2008}, \emph{aquel} is used 5\% of the time, \emph{ese} 26\% and \emph{este} 69\%.
We find a somewhat similar situation in Turkish, where there are also three distinctions in demonstratives (\emph{bu}, \emph{şu}, \emph{o} \citet[67]{schaaik_oxford_2020}\footnote{This is the nominative series. The forms differ for further cases.}). 
...
An increase in morphological information can be understood as an increase in themade explicitly in grammatical form, and thus as an increase in one facet of language complexity in the sense of absolute complexity.
Our metric, described in section \ref{detailed_procedure} and defined precisely in appendix \ref{sec:appendix_ours}, captures the amount of information carried by morphological features across approximately 70 languages spanning 158 datasets from the Universal Dependencies corpora collection v2.14 (UD) \citep{UD_2.14}.
...
Corpora can contain different levels of annotation and meta-data. Datasets in the Universal Dependencies project \citep{UD_2.14} contain information on dependencies, part-of-speech (UPOS), lemma and morphological features.
The last three annotations are of particular interest for discerning morphological information.
Table \ref{tab:turkish_example} is an example of a sentence in Turkish from the UD-dataset Turkish-Penn \citep{kuzgun_2020_UD_turkish_penn} where words are marked for the morphological categories such as case, number, person, aspect and more .
...
In this study, we are not only interested in how many morphological features on average a token in a given language has, but also how informative those features are given their usage frequencies.
For example, the morphological feature category Number appears in the Turkish example with the value Sing (Singular) for the token ``kanun''.
If we look broadly at all tokens in this dataset, we find that this is not unusual; Number has the value Sing (Singular) for 86\% of the tokens on which it occurs.
On the other hand, the feature category Tense shows a more interesting distribution in this dataset: Past 50\%, Present 45\% and Future 5\%.
From an information-theoretic perspective, the morphological feature category Tense is more informative than Number in this dataset.
Put another way, we can more confidently guess at the Number value than we can the Tense value --- Tense surprises us more often.

In our analysis, we separate out the frequencies of morphological features and their values based on UPOS or lemma (henceforth `aggregation level').
We do this for several methodological reasons.
Firstly, a given feature category does not necessarily mean the same thing for tokens of different UPOS. For example, Number does not mean the same thing for a Verb as it does for a Noun, so we ought to treat them differently.
Second, the usage frequencies of the features typically pattern according to UPOS. 
Number is more frequently marked on Nouns than on Verbs. 
Besides UPOS, there can be other classes of words which pattern regularly (e.g. animates versus inanimates, transitive verbs versus intransitive, verbs of action versus verbs of thinking).
Such categories are hard to define cross-linguistically and they are not annotated for in UD-datasets.
Because of this, we use lemma as another aggregation level in addition to UPOS when calculating our metrics.
Third, we believe that aggregating in this manner relates better to real-world language usage.
Users of a language are not facing situations where any feature category can appear for any word, so we should not proceed in that manner in our analysis.
...
\subsection{Data}
We use Universal Dependencies (UD) v2.14 \citep{UD_2.14}, Grambank v1.0 \citep{skirgard_grambank_2023-1, skirgard_grambank_2023} and Glottolog v5.0 \citep{hammarstrom_glottologglottolog_2024}.
We omit 20 UD datasets which lack any morphological feature annotations\footnote{The UD datasets without morphological feautre annotations are:  Akkadian-PISANDUB, Azerbaijani-TueCL, Bavarian-MaiBaam, Chinese-PatentChar, Chukchi-HSE, English-CTeTex,  English-ESLSpok, Frisian\_Dutch-Fame, Irish-TwittIrish, Korean-Kaist, Luxembourgish-LuxBank, Maltese-MUDT, Neapolitan-RB, Old\_Turkish-Clausal, Paumari-TueCL, South\_Levantine\_Arabic-MADAR, Swedish\_Sign\_Language-SSLC, Swiss\_German-UZH, Tagalog-Ugnayan and Vietnamese-VTB.}, as well as three multilingual datasets which would obscure language-specific metrics \footnote{The excluded multilingual datasets are: Turkish\_German-SAGT, Maghrebi\_Arabic\_French-Arabizi and Frisian\_Dutch-Fame.}.

We process the UD-datasets before calculating surprisal (removing empty-nodes, resolving multiwords, inserting dummy-values for missing features etc), which is outlined in detail in appendix \ref{sec:appendix_ours}. We also calculate a modified version of Ç\&R's MFH based on the same dataset as our metrics (see appendix \ref{sec:appendix_ccoltekin} and \ref{sec:appendixComparison}). Finally,  we calculate the Grambank metrics Fusion and Informativity in line with \citet{shcherbakova2023societies} and \cite{skirgard_grambank_2023-1}, with a small modification for how missing data is cropped (see appendix \ref{sec:appendix_rgrambank} and \citet{skirgard_aggregate_2025} for more details).
...
However, unfortunately we cannot include a direct comparison because the original results are based on a small sample and we lack enough details to reproduce their procedure in enough detail.
...
%%% SFM end chopping
We measure the surprisal of morphological features per token and take the average over a dataset to get a point estimate.
...
Given these three binary parameters, we define eight versions of our metric as shown in table \ref{tab:our_metrics_list}.
...
Whereas Ç\&R calculate frequencies on a sample-by-sample basis, and \citet{sproat_database_2014} estimate frequencies in an unknown manner, we calculate frequencies across the entire corpus for each UD dataset.
...
There are several differences between Ç\&R's MFH and ours. We list a few key differences here, for more please see appendix \ref{sec:appendixComparison}.   
...
\item Grambank metrics (for detailed definitions see \citet{skirgard_grambank_2023-1}, \citet{shcherbakova2023societies} and \citet{skirgard_hedvigsrgrambank_2025}) 
    \begin{itemize}
    \item Fusion 
    \item Informativity 
    \end{itemize}
...
First, we consider the relationship between the eight different new metrics defined in this paper. 
...
To save space, we will include only these two in further plots as they represent well the variation of our custom metrics. \footnote{We note that this pair of are not the most distinct in the PUD only comparison (Figure \ref{fig:SPLOM_metric_custom_PUD}), but it does score among the lowest.}
...
We keep these in as the still contain morphological information, with certain thresholds and processing (see appendix \ref{sec:appendix_ours}). 
...
The correlations between our metrics and the Grambank metrics are typically weak or not significant(Figure \ref{fig:SPLOM_metrics_external_grambank_all}).
...
We note also that Ç\&R's MFH also does not correlate with either of the two Grambank metrics.

This lack of correlation highlights the distinctiveness of corpus-based analyses of complexity: insofar as our range of metrics captures something real, it appears to be something that shows up in feature frequencies, rather than the rules describing a language's grammar.
...
Our metrics correlates moderately–strongly with Ç\&R's MFH, but diverges predictably. 
...
Coverage gaps limit geographic interpretations.

In conclusion, mean token-level, UPOS/lemma-conditioned surprisal offers a compact, usage-based estimate of information in morphology --- one facet of morphological complexity. 
...
The python scripts that calculate \cite{ccoltekin2023complexity}'s mean feture entropy take the same input as the R-scripts that calculate our metrics. 
