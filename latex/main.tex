...
Third, we compare our custom metrics to the Grambank metrics and population sizes. The correlations between our metrics and the Grambank metrics are typically weak ($0.45^*-0.48^*$) or not significant (Figures \ref{fig:SPLOM_metrics_external} and \ref{fig:SPLOM_metrics_external_PUD}).
...
This correlation becomes weaker when we consider our lemma-level metric together with population, and disappears entirely when we additionally log-transform the population count.
...
All data in this study is available freely, see the README in the code project for details on versions and access.
