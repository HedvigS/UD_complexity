...
Both `no' and `the' get an XPOS of DT in UD\_English-GENTLE, so they would still receive each other's features as dummy values when aggregating at that level.
...
\begin{itemize}
    \item (Do we have an idea how many of our corpora this would work for?)
\end{itemize}
...
\begin{itemize}
    \item Aggregation across corpora with different annotation standards (Rev ID 15): negative affects the comparability of our metric because features may be annotated differently.
    \item Emojis, percentages, etc.
\end{itemize}
