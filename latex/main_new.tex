...
\subsection{Dummy feature assignment at the UPOS level}

In order to account for the fact that a lack of marking can carry morphological information, we assign `dummy' features to tokens of the same aggregation level (UPOS/lemma) based on certain rules.
...
In such a case we would assign to all members of one subgroup dummy values of features that belong properly to another.
...
But it intuitively it may seem odd to describe `the' as carrying information about Polarity.
The tokens receive this dummy feature assignment (Polarity=unassigned) because another tokens of the same UPOS (DET) carry this feature. 
...
And when `no' is encountered, the negative Polarity is much more surprising and contributes more significantly to that token's information quantity.
...
Our basic position is that information can be carried by the absence of morphological marking and that it is relevant to standardise within groups (UPOS/lemma) -  this is what dummy assignment is designed to address.
...
Some treebanks have a sizeable proportion of missing lemmas.
This is a problem for several of our metrics.
Not only do we report the total number of distinct lemmas and the lemma-token ratio, four of our custom metrics treat lemma as the aggregation level, which requires defining feature classes across tokens belonging to the same lemma.
There is no determinate way to assign features to tokens with missing lemmas, so no way to consistently apply these six metrics without making further decisions.

We ameliorate this problem in two ways, both of which can affect comparability.
First, we simply omit those six metrics for datasets with more than 40\% missing lemmas.
...
We chose this 40\% threshold because there is a big jump from TODO to TODO missing lemmas across the treebanks we are dealing with.
Manual inspection of a few treebanks with less than 40\% missing lemmas suggests many of these tokens are forms that don't inflect anyway.
They have typically been left out because the lemma would be considered identical to the token.
...
In the first case, we lose the ability to compare treebanks with many missing lemmas across the six metrics that rely on lemma presence.
The power of statistical tests for correlations involving these metrics is therefore reduced.
In the second case, we might be inadvertently adding lemmas to tokens which do not properly have lemmas.
This would skew both the number of lemmas and the lemma-token ratio.
Different decisions might reasonably be made here, but the aforementioned manual checks plus the relatively low level of missing lemmas in most datasets means that we would expect the negative impact of our procedure to be small.

\subsection{Inconsistent annotation standards across corpora}

The UD project provides documentation guiding treebank annotation.
Nevertheless there are different levels of rigour and different choices made among the maintainers of different treebanks, even among those drawn from corpora of the same language.
We deal with the most significant issues of this kind by altogether excluding some treebanks from our analysis.
For example, Bavarian-MaiBaam has no feature annotations at all. 
We exclude treebanks with no features because our informational measure requires feature annotations to produce a meaningful result.

However, there are remaining differences that could negatively impact the comparability of our metric.
Features may be annotated with slightly different standards across treebanks, even treebanks of the same language.
...
If the PUD dataset were so annotated, those words would get different values for our metric than they in fact do. 
Conversely if the English-GENTLE dataset were not marked for these values, those tokens' informational quantities on our metric would also be different from those we actually calculated.
This is bad: different annotation decisions has led to inconsistent data which means the values we have calculated are not as comparable as they could be.
...
For more details on genre and comparability see appendix \ref{subsec:caveat_comparability}.

A broader point worth making is that any workflow downstream of a large database will be hostage to that database's consistency. 
...
Actually performing analyses like this one and that of \citet{ccoltekin2023complexity} can help reveal inconsistencies and increase the motivation for more rigorous and consistent data gathering in the first place. 
...
(Contrariwise, if nobody ever attempted to analyse the data there would be little motivation to improve it -- we have to start somewhere, with the promise of getting better as we go along.)

Finally in this section, while we have removed certain symbols such as emojis from each dataset before the calculation step, we elected to leave in certain other symbols such as ampersands (\&), hashes (\#) and at-signs (@).
...
See appendix \ref{subsec:caveat_comparability} for more detailed information about comparability challenges in UD.
...
\item Remove tokens with no lemma (i.e. with no value in the `lemma' field; this includes removing multiword tokens) % line 145 of mlc_morph.py
\item Remove tokens with no value in the `token' field % line 145 of mlc_morph.py 
\item Remove tokens that have the UPOS NUM and where the token is non-alphabetical (i.e. ``50'') % line 142 of mlc_morph.py
\item Remove tokens with no morphological features (i.e. with no value in the `feats' field) % lines 281 and 336 of mlc_morph.py
...
We did not do the bootstrapping sampling (sample sentences with replacement until you reach a given token target) - instead we ran the analysis on the exact same datasets as ours - i.e all datasets in the 173 UD datasets with more than 13,000 tokens (excluding tokens with UPOS PUNCT or X). 
...
In this study, we work with a larger number of datasets where some have fewer tokens (>13,000).
Note that the datasets serving as the input have more than 13,000 tokens, but filters in the Ç\&R-procedure eliminate further tokens.
...
Removing bootstrapping also included removing the calling of the function sample\_nodes in get\_mfh, which had the consequence that some of the filtering in sample\_nodes happen in get\_mfh instead.
...
\begin{enumerate}
\item (\texttt{collapse\_UD\_dirs}) Exclude datasets where the README.md states that Features are not available
...
\item (\texttt{process\_UD\_data()}) Make all tokens lower-case.
...
\item Replace missing lemmas with the token. There is variation in how datasets of UD treat certain tokens in terms of defining or not defining a lemma. For example, many proper nouns (PROPN) or adpositions (ADP) will lack a lemma annotation. Some datasets, like Arabic-NYUAD, leave the lemma field empty for most tokens where UPOS is ADP or PROPN. Conversely, in Greek-GUD all items of the same UPOS values have a defined lemma (majority of the time identical to the token). To normalise for our comparison, we replace missing lemmas with the content in the token field. At this stage we also keep track of what proportion of lemmas were missing, because certain metrics are not calculated on datasets with greater than 40\% missing lemmas (see below and section \ref{sec:limitations}).
...
\item (\texttt{calculate\_surprisal()}) Count the number of unique feature categories.
...
\item \textit{Separated features}: Count how many times each feature \textit{value} occurs \textit{for that feature category, in tokens of the same UPOS/lemma} (column `Count' in Table \ref{tab:unassigned_ex_SPLIT}). \textit{Featstring}: Count how many times each \textit{featstring} occurs, in tokens of the same UPOS/lemma.
...
Using subclass as an aggregation level gives very similar results to using 
