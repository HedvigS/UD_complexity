...
Language complexity\footnote{Unfortunately, the terms ``simple'' and ``complex'' in relation to language have in some circles acquired value judgment, with so called ``complex'' languages being interpreted as more admirable or better in some other way \citep{sampson2009linguistic}. 
...
\subsection{Data}
We use Universal Dependencies (UD) v2.14 \citep{UD_2.14} . We are grateful for all the labour of UD-maintainers and cite here the relevant dataset-specific publications where available: 

\begin{itemize}
    \item Akkadian-RIAO \citep{luukko_akkadian_2020}
    \item Ancient\_Greek-Perseus \citep{bamman_ancient_2011, celano_ancient_2014}
    \item Ancient\_Greek-PROIEL \citep{haug_creating_2008}
    \item Ancient\_Hebrew-PTNK \citep{swanson_universal_2022}
    \item Arabic-PADT \citep{hajic_prague_2009, smrz_prague_2008} 
    \item Armenian-ArmTDP \citep{yavrumyan_armtdp_2017, yavrumyan_universal_2019, yavrumyan_universal_2020}
    \item Armenian-BSUT \citep{yavrumyan_universal_2020}
    \item Basque-BDT \citep{aranzabe_automatic_2015}
    \item Belarusian-HSE \citep{shishkina_sculpting_2021}
    \item Bulgarian-BTB \citep{osenova_btb-tr05_2004, simov_practical_2003, simov_hpsg-based_2002, simov_btb-tr03_2004, simov_design_2005}
    \item Catalan AnCora \citep{taule_ancora_2008}
%    \item Chinese-Beginner %can't tell how to cite this dataset properly
    \item Classical\_Chinese-Kyoto \citep{yasuoka_universal_2019, yasuoka_designing_2022} %&here's where hedvig left off
    \item Coptic-Scriptorium \citep{zeldes_coptic_2018}
    \item Croatian-SET \citet{agic_universal_2015}
    \item English-EWT \citep{silveira_gold_2014}
    \item English-GUM \citet{zeldes_gum_2017}
    \item English-GUMReddit \citep{behzad_cross-genre_2020}
    \item Erzya-JR \citep{rueter_towards_2018}
    \item Galician-PUD \citep{sanchez-rodriguez_increasing_2024}
    \item Hebrew-HTB \citep{tsarfaty_unified_2013, mcdonald_universal_2013}
    \item Hebrew-IAHLTwiki \citep{zeldes_secondwave_2022}
    \item Hindi-HDTB \citep{bhat_hindiurdu_nodate, palmer_hindi_2009}
    \item Italian-Old \citep{corbetta_highway_2023, alighieri_commedia_1994}
    \item Italian-PoSTWITA \citep{sanguinetti_postwita-ud_2018, sanguinetti_treebanking_2023, sanguinetti_annotating_2017, bosco_overview_2016}
    \item Low\_Saxon-LSDC \citep{siewert_low_2024}
    \item North\_Sami-Giella \citep{sheyanova_annotation_2017}
    \item Old\_East\_Slavic-Birchbark
\end{itemize}
