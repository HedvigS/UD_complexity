...
In order to account for the fact that a lack of marking can carry morphological information, we assign `dummy' features to tokens based on certain rules.
...
That's because UPOS is a broad church: different subgroups of token within the same part-of-speech may be associated with very different features.
In such a case we would assign to all members of one subgroup dummy values of features that belong properly to another.

For example, tokens of the lemma `the' in the corpus UD\_English-GENTLE when processed at aggregation level UPOS for core features only get the following feature values:
...
But it intuitively makes no sense to describe `the' as carrying information about, say, Polarity.
The token receives this dummy feature because another token of the same UPOS -- in this case, `no' -- has a determinate value for Polarity -- in this case, `Neg'.
...
It might be objected that the negativity conveyed by the token `no' is in the semantics rather than morphologically, and thus our metric is assigning information to morphology what properly belongs to the bare meaning of the word.
Insofar as this is true, it is a question of the tagging conventions being enforced more rigorously.
Polarity is listed as an `Inflectional feature' in the UD documentation.
If it is being used to flag semantic information, we would consider that an incorrect usage of the field.
We have made no attempt to correct what might be seen as mistakes in the treebanks themselves: we have only established a workflow designed to use correctly formatted treebanks to estimate morphological information quantities.
Errors or infelicities in the data should be fixed upstream, not in our workflow itself.

Given the foregoing considerations, one might think that UPOS is too high a level at which to aggregate.
Fortunately, some treebanks offer a more fine-grained categorisation of tokens between the level of lemma and UPOS.
Some corpora use the XPOS field to record more fine-grained classifications of part-of-speech. 
UD\_English-GENTLE seems to do it; though the documentation doesn't say exactly what guidelines they followed, it appears similar to that of UD\_English-ESLSpok whose documentation explicitly cites the Penn Treebank Tagset guidelines.
Aggregating at XPOS would ameliorate some of the overeager dummy feature assignment that happens at the UPOS level.
However, it wouldn't change the result of the example we've just looked at.
Both `no' and `the' get the XPOS `DT' (determiner) in UD\_English-GENTLE, so they would still receive each other's features as dummy values when aggregating at that level.
Furthermore, not all treebanks use the XPOS option, and those that do don't all use it in the same way.
So this potential improvement would have to be looked at carefully in future developments of the current metric. % See pycode_ud/sandbox/notebooks/count_xpos.ipynb for an initial analysis of the statistics on the XPOS column across treebanks.

In sum, dummy feature assignment at the UPOS level can lead to unintuitive attribution of morphological features to tokens.
Our basic position is that information can be carried by the absence of morphological marking, and that is what dummy assignment is designed to capture.
Cases where this is not so might include semantic information being illicitly attributed to morphological features.
This is a problem for the treebank maintainers, and not something to be fixed in our workflow.
We take it on faith that the features intended to capture morphological aspects of tokens do so.
A more fine-grained aggregation option exists at the XPOS level for some treebanks.
We leave to future work the possibly of estimating morphological information when aggregating at this level.
...
For example, UD\_Bavarian-MaiBaam has no feature annotations at all. 
...
For example, UD\_English-PUD does not assign a value for the feature Case to words such as `our' and `their', while UD\_English-GENTLE assigns the case `Gen' (for genitive). 
...
Conversely if the GENTLE dataset were not marked for these values, those tokens' informational quantities on our metric would also be different from those we actually calculated.
...
A clear outlier is rather UD\_English-Atis whose surprisal values are markedly lower than any of the other English datasets.
...
\begin{itemize}
    \item TODO: Emojis, percentages, etc.
\end{itemize}
