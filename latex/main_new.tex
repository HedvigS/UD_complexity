...
We also use Glottolog v5.0 \citep{hammarstrom_glottologglottolog_2024} for information about languages such as their location.
...
We acknowledge that ``lemma'' is as an elusive category psycholinguistically  \citep{meyer_elusive_2026}, but in this study we believe it contributes a meaningful way of structuring the lexicon at a finer level compared to UPOS and is preferable to treating all tokens are belonging to one group.
If another way of grouping tokens is preferable, we are happy to re-apply our approach with another aggregation level --- also to other datasets (see appendix \ref{appendix:applying_to_non_UD}).
...
The second concerns whether we only use all features in a dataset, or only those centrally defined (`core'' features as defined by the UD project (see appendix \ref{subsec:caveat_comparability} and \ref{sec_appendix:core_features}).
...
\FloatBarrier
\noindent The pair of custom metrics that differ the most from each other in the comparison with all datasets are\footnote{We note that there is another pair in the larger dataset (not PUD) that are as uncorrelated (0.85*): mean sum surprisal of features, aggregation = UPOS, core features vs mean sum surprisal of featstring, aggregation level = lemma, all features. As these pairs are very comparable and the pair discussed in the main text is also among the less correlated in the PUD dataset, we will focus on the other pair.}:
\begin{itemize}
...
The correlations between our metrics and mean feature entropy are moderate to strong (0.64*-0.82*).
For further details of the conceptual differences between the metrics, see section \ref{sec:CR_comparison} and supplementary material \ref{sec:appendix_ccoltekin} and \ref{sec:appendixComparison}.

Firstly, there were whole datasets and tokens within datasets for which it was not possible to assign a MFH-value but where we can assign morpho-surprisal values. 
This is due to differences in data processing decisions, such as how to treat tokens with no morphological features or missing lemmas.
...
We keep these in as they still contain morphological information (see appendix \ref{sec:appendix_ours}). 
This make a difference for example for isolating languages like Mandarin Chinese, as only tokens with morphology are taken into account for computing MFH.
We can see the consequences of this if we compare Mandarin Chinese with Portuguese, which are commonly described as having little and moderate morphology respectively.
If we rank UD-datsets by their MFH values, then Portuguese-PUD appears lower than Chinese-PUD (7 versus 9) - less information in the morphology. 
...
We assign to all members of one subgroup dummy values of features that belong properly to another.

For example, tokens representing the word `the' in the corpus English-GENTLE get the following feature values when processed at aggregation level UPOS and only considering core features:
...
The tokens receive this dummy feature assignment (Polarity=unassigned) because another token of the same UPOS (DET) carry this feature. 
In this case, it is specifically the word `no' which has a value for Polarity (Neg).
Since some determiners are explicitly marked for polarity and others aren't, in order to quantify the amount of information conveyed about polarity on encountering a determiner we must capture the distinction with this unassigned dummy feature.
...
We note however that the morpho-surprisal scores when aggregating over UPOS and lemma are strongly correlated, suggesting that this issue does not of major concern.  
...
Second, for datasets with less than 40\% missing lemmas we impute missing lemmas with the token value (plus underscore UPOS, as with all other lemmas).
We chose this threshold because there is a jump from 32\% to 58\% missing lemmas across the datasets in question.
Manual inspection of a few  datasets with less than 40\% missing lemmas suggests many of these tokens are forms that don't inflect anyway. %#TODO
...
For example, UD\_Finnish-TDT uses the hash character as a separator between the constituent parts of compound words.
Such symbols could cause problems if they are used inconsistently, but we would estimate the effect on our results to be minor or negligible.
...
\label{appendix:applying_to_non_UD} %#todo add subclass values
...
But we do provide advice and our code in case other researchers want to try.
...
The function \texttt{calculate\_surprisal()} can use any column name in the input table for grouping tokens for the calculations, it needn't be UPOS or lemma. 
...
Ukranian-IU &  001\_0002\_1	&	\cyrillicfont{У}	&	\cyrillicfont{у}\_ADP	&	Case=Loc	&	ADP	\\	\midrule
Ukranian-IU & 001\_0002\_10	&	\cyrillicfont{зображення}	&	\cyrillicfont{зображення}\_NOUN	&	Animacy=Inan| Case=Nom| Gender=Neut|Number=Sing	&	NOUN	\\	\midrule
Ukranian-IU & 001\_0002\_11	&	\cyrillicfont{Венери}	&	\cyrillicfont{Венера}\_PROPN	&	Animacy=Anim| Case=Gen|Gender=Fem| NameType=Giv|Number=Sing	&	PROPN	\\	\midrule
Ukranian-IU & 001\_0002\_12	&	\cyrillicfont{та}	&	\cyrillicfont{та}\_CCONJ	&	unassigned=unassigned	&	CCONJ	\\	\midrule
Ukranian-IU & 001\_0002\_13	&	\cyrillicfont{Адоніса}	&	\cyrillicfont{Адоніс}\_PROPN	&	Animacy=Anim|Case=Gen| Gender=Masc| NameType=Giv|Number=Sing	&	PROPN	\\	\midrule
Ukranian-IU & 001\_0002\_2	&	\cyrillicfont{домі}	&	\cyrillicfont{дім}\_NOUN	&	Animacy=Inan|Case=Loc| Gender=Masc| Number=Sing	&	NOUN	\\	\midrule
Ukranian-IU & 001\_0002\_3	&	\cyrillicfont{римського}	&	\cyrillicfont{римський}\_ADJ	&	Case=Gen|Gender=Masc| Number=Sing	&	ADJ	\\	\midrule
Ukranian-IU & 001\_0002\_4	&	\cyrillicfont{патриція}	&	\cyrillicfont{патрицій}\_NOUN	&	Animacy=Anim|Case=Gen| Gender=Masc|Number=Sing	&	NOUN	\\	\midrule
Ukranian-IU & 001\_0002\_5	&	\cyrillicfont{Руфіна}	&	\cyrillicfont{Руфін}\_PROPN	&	Animacy=Anim|Case=Gen| Gender=Masc|NameType=Giv| Number=Sing	&	PROPN	\\	\midrule
Ukranian-IU & 001\_0002\_6	&	\cyrillicfont{була}	&	\cyrillicfont{бути}\_VERB	&	Aspect=Imp|Gender=Fem| Mood=Ind|Number=Sing| Person=unassigned|Tense=Past| VerbForm=Fin	&	VERB	\\	\midrule
Ukranian-IU & 001\_0002\_7	&	\cyrillicfont{прегарна}	&	\cyrillicfont{прегарний}\_ADJ	&	Case=Nom|Gender=Fem| Number=Sing	&	ADJ	\\	\midrule
Ukranian-IU & 001\_0002\_8	&	\cyrillicfont{фреска}	&	\cyrillicfont{фреска}\_NOUN	&	Animacy=Inan|Case=Nom| Gender=Fem|Number=Sing	&	NOUN	\\	\midrule
\bottomrule
    \end{tabular}
\end{table}
...
In our analysis, we treat tokens with missing lemmas as genuine tokens. 
We replace missing lemmas with the content in the token field in order to make it possible to include these in the analysis.
There are three datasets \footnote{English-GUMReddit, Thai-PUD and Faroese-FarPaHC} which lack lemmas altogether. 
For Ç\&R MFH-procedure, these tokens are all filtered out leaving 0 tokens and a MFH value of 0.
We treated the MFH value for these three datasets as missing instead of as 0.
...
This is for example the case with the dataset discussed earlier; Arabic-NYUAD. 
We believe these datasets still contain valuable information for the computation of morphological complexity and hence include them.
We do however exclude them from some analysis, such as TTR.
If tokens \emph{and} lemmas are both missing then the lemma category becomes just the UPOS (because we merge lemmas with UPOS, for example to distinguish between mark (VERB) and mark (NOUN); eg. `\_VERB'.
...
However, for our metric, after assigning ``dummy'' feature values  (e.g. `NumType=unassigned') given features of other tokens in the same UPOS/lemma, we insert a ``dummy'' feature (`unassigned=unassigned') for tokens that still lack features.
...
We take that to mean that this token ``30'' is identical to ``{\cyrillicfont тридцать}'' (Cyrillic spelling of the cardinal numeral thirty in accusative form) and should be included when it comes to calculations of morphological features.
...
For example, some languages do not have an identifiable class of adjectives, with such meanings instead being expressed with nouns or verbs and some languages have a fluid boundary between what we might call nouns and verbs. 
...
Consider the token {\cyrillicfont цIитI} in Table \ref{tab:Abaza_sent_10} and {\cyrillicfont йазкIкIыта} in Table \ref{tab:Abaza_sent_11} which are both marked as UPOS VERB. 
The first token, {\cyrillicfont цIитI}, is annotated for features often associated with finite verbs such as tense. 
...
Based on studying these annotations alone, we reach the conclusion that there are at least two groups of tokens which are both included under the UPOS VERB and that have very different morphological behaviour in this dataset.
...
\begin{table}[]
    ...
    Abaza     & {\cyrillicfont{щарда \underline{цIитI} ари агIаншижьтара}} \\    \midrule
    Russian    & {\cyrillicfont{С тех пор прошло много времени }}\\ 
    English & A lot of time has passed since then\\    \midrule
...
\begin{table}[]
    ...
    \begin{tabular}{p{3cm}|p{8cm}}
    Abaza     & {\cyrillicfont{апхъа азаман уагIахъата адунай йыквыз шабгаз \underline{йазкIкIыта} зджьара йбзазун }}\\  \midrule
    Russian    &{\cyrillicfont{В то время люди, которые находились на свете, собравшись, жили где-то }}\\  \midrule
...
This standardized definition therefore still leaves a lot of flexibility, as may be desirable and expected in a project of this kind.
...
We also tested out an experimental approach to addressing this issue: defining groups of tokens within the same UPOS based on consistent patterns of morphological features.
...
For example, this would mean treating all tokens with the UPOS verb that are marked for Tense and VerbForm in Abaza-UTB as a separate group from all verbs which are marked for Gender, Number[abs], Number[io], Person[abs] and Person[io]. 
We call this grouping ``subclass'' and they can be thought of as a level below UPOS but above lemma. 
Because of the way these subclasses are defined, there is no dummy-feature value insertion because all tokens of the same subclass share exactly the same feature categories.
This approach is experimental, and it relies on the assumption that the systemic patterns of feature categories within UPOS of the same dataset are a meaningful unit. 
As this is a novel and experimental approach, we do not discuss this in the main text.
We computed the morpho-surprisal scores for each dataset using subclass as an aggregation level, in addition to UPOS and lemma, varying all features / core features and separated feature / featstring --- making for an additional four morpho-surpisal scores.
Figures \ref{fig:SPLOM_metric_custom_plus_subclass} and \ref{fig:SPLOM_metrics_custom_PUD_plus_subclass} shows the correlations between all 12 morpho-surpiscal scores, including those using subclass as an aggregation level.
Using subclass as an aggregation level gives very similar results to using TODO FINISH
...
\toprule
    ...
    \midrule
morpho-surprisal /   feat /   UPOS /   all features	&	TTR	&	0.000005	&	0.000019	\\ \midrule
...
morpho-surprisal /   featstring  /    lemma /   core features only	&	TTR	&	0.103477	&	0.103477	\\ \midrule
...
TTR	&	\# Feat cat /   all features	&	0.007944	&	0.015888	\\ \midrule
TTR	&	Ç\&R's MFH (slightly modified version)	&	0.004723	&	0.014168	\\ \bottomrule
...
\toprule
	\textbf{Correlate 1}	&	\textbf{Correlate 2}	& \textbf{p-value (unadjusted)}    &	\textbf{p-value (adjusted)}	\\
    \midrule
morpho-surprisal /   feat /   UPOS /   all features	&	TTR	&	0.000944	&	0.006605	\\ \midrule
morpho-surprisal /   feat /   UPOS /   all features&	\# Feat cat /   all features	&	0.001600	&	0.009599	\\ \midrule
morpho-surprisal /   feat /   UPOS /   all features	&	Ç\&R's MFH (slightly modified version)	&	0.000024	&	0.000192	\\ \midrule
morpho-surprisal /   featstring  /    lemma /   core features only	&	TTR	&	0.034227	&	0.078336	\\ \midrule
morpho-surprisal /   featstring  /    lemma /   core features only&	\# Feat cat /   all features	&	0.004494	&	0.017973	\\ \midrule
morpho-surprisal /   featstring  /    lemma /   core features only	&	Ç\&R's MFH (slightly modified version)	&	0.003253	&	0.016263	\\ \midrule
TTR&	\# Feat cat /   all features	&	0.107420	&	0.107420	\\ \midrule
TTR	&	Ç\&R's MFH (slightly modified version)	&	0.026112	&	0.078336	\\ \bottomrule
\end{tabular}
\end{table}
...
morpho-surprisal /   feat /   UPOS /   all features	&	Informativity (Grambank)	&	0.336824	&	1.0000000	\\ \midrule
morpho-surprisal /   featstring  /    lemma /   core features only	&	Fusion (Grambank)	&	0.006206	&	0.062059	\\ \midrule
morpho-surprisal /   featstring  /    lemma /   core features only	&	Informativity (Grambank)	&	0.449203	&	1.0000000	\\ \midrule
TTR	&	Fusion (Grambank)	&	0.761930	&	1.0000000	\\ \midrule
TTR	&	Informativity (Grambank)	&	0.377411	&	1.0000000	\\ \midrule
\# Feat cat /   all features	&	Fusion (Grambank)	&	0.425756	&	1.0000000	\\ \midrule
\# Feat cat /   all features	&	Informativity (Grambank)	&	0.938133	&	1.0000000	\\ \midrule
Ç\&R's MFH (slightly modified version)	&	Fusion (Grambank)	&	0.093866	&	0.750925	\\ \midrule
Ç\&R's MFH (slightly modified version)	&	Informativity (Grambank)	&	0.486198	&	1.0000000	\\ \bottomrule
...
morpho-surprisal /   feat /   UPOS /   all features	&	Informativity (Grambank)	&	0.639450	&	1.0000000	\\ \midrule
morpho-surprisal /   featstring  /    lemma /   core features only	&	Fusion (Grambank)	&	0.060927	&	0.548342	\\ \midrule
morpho-surprisal /   featstring  /    lemma /   core features only	&	Informativity (Grambank)	&	0.529688	&	1.0000000	\\ \midrule
TTR	&	Fusion (Grambank)	&	0.350025	&	1.0000000	\\ \midrule
TTR	&	Informativity (Grambank)	&	0.840792	&	1.0000000	\\ \midrule
\# Feat cat /   all features	&	Fusion (Grambank)	&	0.389697	&	1.0000000	\\ \midrule
\# Feat cat /   all features	&	Informativity (Grambank)	&	0.849674	&	1.0000000	\\ \midrule
Ç\&R's MFH (slightly modified version)	&	Fusion (Grambank)	&	0.109941	&	0.879529	\\ \midrule
Ç\&R's MFH (slightly modified version)	&	Informativity (Grambank)	&	0.259548	&	1.0000000	\\ \midrule
...
PUD datasets are translational equivalents, meaning that statistics such as TTR/\# types and number of feature categories are more likely to track relative morphological richness across datasets compared to other texts.
