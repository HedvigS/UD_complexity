...
if(!"GB203b" %in% ParameterTable$ID){
  ...
  if(  !(Fusion_option %in% c("count_zero_half_and_one", "count_one_only", "count_one_and_half"))){
    ...
    Fusion_df <- ValueTable %>%
        ...
        dplyr::mutate(n = n()) %>%
        dplyr::filter(n >= n_fusion_feats * missing_cut_off) %>%
    ...
    lg_df_for_flex_count <- ValueTable  %>%
        ...
        dplyr::group_by(Language_ID) %>%
        dplyr::mutate(n = n()) %>%
        dplyr::filter(n >= n_flexivity_feats * missing_cut_off) %>%
    ...
    lg_df_for_HM_DM_count <- ValueTable %>%
        ...
        dplyr::group_by(Language_ID) %>%
        dplyr::mutate(n = n()) %>%
        dplyr::filter(n >= n_locus_marking_feats * missing_cut_off) %>%
        # reversing the Values of the features that have a score of 0
    ...
    lg_df_for_gender_nc_count <- ValueTable  %>%
        ...
        dplyr::group_by(Language_ID) %>%
        dplyr::mutate(n = n()) %>%
        dplyr::filter(n >= n_gender_NC_feats * missing_cut_off) %>%
        # reversing the Values of the features that have a score of 0
     ...
     lg_df_for_OV_VO_count <- ValueTable  %>%
        ...
        dplyr::group_by(Language_ID) %>%
        dplyr::mutate(n = n()) %>%
        dplyr::filter(n >= n_word_order_feats * missing_cut_off) %>%
    ...
    lg_df_informativity_score <-  ValueTable  %>%
        ...
        dplyr::group_by(Language_ID) %>%
        dplyr::mutate(n = n()) %>%
        dplyr::filter(n >= n_informativity_feats * missing_cut_off) %>%
