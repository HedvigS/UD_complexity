...
all_counts <- all_counts %>% 
  ...
  dplyr::mutate(diff_empty = n_tokens_in_input  - n_tokens_empty_dropped)
...
#  dplyr::filter(diff > 6000) %>% 
  ...
  ggplot2::ggplot() +
  ggplot2::geom_point(mapping = ggplot2::aes(x = dir, y = value, fill = variable, color = variable),shape = 18, stroke = 0, alpha = 0.2, size = 2) +
...
ggplot2::ggsave("output/plots/token_count_comparisons.png", height = 15, width = 10)
